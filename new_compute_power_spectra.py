...
def get_bispectrum_constraint(fsky, mm, mmm, ell0Max, Clnoise, Clgg, Blll):
    ...
    return 0.21 / (fsky * constraint)**0.125
...
def sigma_screening(epsilon4, fsky, ellmin, ellmax, screening, leftover):
    ...
    return 0.21 / (fsky * np.sum(TrF))**0.125
...
def sigma_screeningVunWISE(ep2, fsky, ellmin, ellmax, cltauscreening, leftover, clgaltau, clgalgal):
    ...
    return 0.35 / (fsky * np.sum(TrF))**0.25
